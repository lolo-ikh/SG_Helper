...
CREATE OR REPLACE FUNCTION checkin_attendee(p_meeting_id BIGINT, p_name TEXT)
RETURNS JSON AS $$
  INSERT INTO meeting_checkins (meeting_id, name)
  VALUES (p_meeting_id, p_name)
...
RETURNS JSON AS $$
  SELECT COALESCE(json_agg(json_build_object('name', name, 'checked_in_at', checked_in_at)), '[]'::json)
